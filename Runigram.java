...
public class Runigram {
	...
	public static Color[][] read(String fileName) {
		...
		for (int i = 0; i < numCols; i++) {
			for (int u = 0; u < numRows; u++) {
				...
				int blue = in.readInt();
				image[i][u] = new Color(red, green, blue);
			}
		}
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		for (int i = 0; i < image.length / 2; i++) {
			for (int u = 0; u < image[i].length; u++) {
				...
				image[i][u] = image[image.length - i -1][u];
				image[image.length - i -1][u] = temp;
			}
		}
		return image;
	}
	...
	public static Color[][] flippedVertically(Color[][] image){
		for (int i = 0; i < image[0].length / 2; i++) {
			for (int u = 0; u < image.length; u++) {
				...
				image[u][i] = image[u][image[0].length - i - 1];
				image[u][image[0].length - i - 1] = temp;
			}
		}
		return image;
	}
	...
	private static Color luminance(Color pixel) {
		int lumen = (int)(.299*pixel.getRed() + .587*pixel.getGreen() + .114*pixel.getBlue());
		return new Color(lumen, lumen, lumen);
	}
	...
	public static Color[][] grayScaled(Color[][] image) {
		Color[][] changedImage = image.clone();
		for (int i = 0; i < image.length; i++) {
			for (int u = 0; u < image[i].length; u++) {
				changedImage[i][u] = luminance(image[i][u]);
			}
		}
		return changedImage;
	}	
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i = 0; i < scaledImage.length; i++) {
			for (int u = 0; u < scaledImage[i].length; u++) {
				scaledImage[i][u] = image[(i * image.length) / height][(u * image[0].length) / width];
			}
		}
		return scaledImage;
	}
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		for (int i = 0; i <= n; i++) {
			target = blend(source, target, (n-1)/n);
			display(target);
			StdDraw.pause(500);
		}
	}
